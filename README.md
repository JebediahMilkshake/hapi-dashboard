...
The "What's for Dinner?" panel displays 3 meal suggestions generated by a local Ollama LLM. Each card shows the meal name, estimated cook time, and difficulty rating (easy/medium/hard).
...
- No cloud AI services — runs entirely on your local network
...
Pi Browser  ──fetch──>  Home Assistant REST API  (weather, calendars, shopping, theme)
            ...
            ──fetch──>  Ollama API  (dinner suggestions, local network)
```
...
- For dinner suggestions: [Ollama](https://ollama.ai) running on your network with a model pulled (e.g., `ollama pull llama3.1`)
...
### 4. Ollama CORS (for dinner suggestions)

If Ollama runs on a different host than HA, you need to allow cross-origin requests:
